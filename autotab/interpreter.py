"""Helpers to inspect, sonify and plot GuitarSet .jams annotations.

Optional extras: ``pretty_midi`` (+ fluidsynth) for sonification. Install with
...
def save_small_wav(out_path, y, fs):
    """Write a 16-bit PCM wav (replaces the removed librosa.output.write_wav + sox)."""
    sf.write(out_path, y, fs, subtype="PCM_16")


def jams_to_midi(jam, q=1):
    # q = 1: with pitch bend. q = 0: without pitch bend.
    import pretty_midi  # optional dependency
    ...
    for anno in annos:
        ...
        for note in anno:
            pitch = round(note.value)
            bend_amount = round((note.value - pitch) * 4096)
            st = note.time
            dur = note.duration
            n = pretty_midi.Note(
                velocity=100 + np.random.choice(range(-5, 5)), pitch=pitch, start=st, end=st + dur
            )
...
def visualize_jams_pt(jam, save_path=None):
    ...
    # plot pitch
    for s, string_tran in enumerate(annos_pt):
        handle_list.append(mlines.Line2D([], [], color=style_dict[s], label=string_dict[s]))
        df = string_tran.to_dataframe()
        pitch_s = df.value.apply(lambda x: librosa.hz_to_midi(float(x["frequency"])))
        pitch_s.name = "pitch"
        df = pd.concat([df, pitch_s], axis=1)
        plt.scatter(df.time, df.pitch, s=0.1, color=style_dict[s], label=string_dict[s])

    # plot Beat
    anno_b = jam.search(namespace="beat_position")[0]
    handle_list.append(mlines.Line2D([], [], color="k", label="downbeat"))
    handle_list.append(mlines.Line2D([], [], color="k", linestyle="dotted", label="beat"))
...
def tablaturize_jams(jam, save_path=None):
    ...
    for s, string_tran in enumerate(annos):
        ...
        for note in string_tran:
            start_time = note[0]
            midi_note = note[2]
            fret = round(midi_note - str_midi_dict[s])
            plt.scatter(start_time, s + 1, marker=f"${fret}$", color=style_dict[s])
    ...
    plt.ylabel("String Number")
    # plt.title(jam.file_metadata.title)
    plt.legend(loc="upper center", bbox_to_anchor=(0.5, -0.2), handles=handle_list, ncol=8)
    plt.xlim(-0.5, jam.file_metadata.duration)
    # fig.set_size_inches(6, 3)
    if save_path:
        plt.savefig(save_path)
...
def visualize_chords(jam, save_path=None):
    ...
    for chord in chord_ann.data:
        plt.axvline(chord.time, linestyle="dotted", color="k", alpha=0.5)
...
def add_annotations_to_barline(
    ax, annotations, beat_annotations, ygrow_ratio=1, label_xoffset=0, label_yoffset=0
):
    """
...
def add_annotations(ax, annotations, ygrow_ratio=0, label_xoffset=0, label_yoffset=0):
    """
